...
def print_test_info():
    ...
    print("4. (manual) Create a new branch with a timestamp name.")
    print("5. (manual) Make some changes to the file in the new branch.")
    print("6. (manual) Commit the changes to the new branch.")
    print("7. (manual) Merge the new branch into the main branch.")
    print("8. (manual) Observe that the merge results are correct.")
...
def check_if_file_exists():
    # Check if the file exists, exiting if it does.
    if os.path.exists(filename):
        print(f"File '{filename}' already exists, proceeding.")
    else:
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        ...
        if sys.argv[1] == "--help":
            ...
            create_directory_if_dne(directory_name)
